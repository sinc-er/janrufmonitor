...
public class IncomingCall extends AbstractHandler implements IEventSender {
	...
	private class HandlerThread implements Runnable {
		...
		public void run() {
			if (getRuntime().getMsnManager().isMsnMonitored(
				...
				IEvent ev = eventBroker.createEvent(IEventConst.EVENT_TYPE_INCOMINGCALL, this.m_c, eventCond);
				eventBroker.send(this.m_s, ev);  
				m_logger.info("Send incoming call event.");
	...
	public void handleWithException(IHttpRequest req, IMutableHttpResponse resp) throws HandlerException {
		...
		try {
			...
			ICip cip = PIMRuntime.getInstance().getCallFactory().createCip(
				req.getParameter(IncomingCall.PARAMETER_CIP),
				""
			);
			
			IMsn msn = PIMRuntime.getInstance().getCallFactory().createMsn(
				req.getParameter(IncomingCall.PARAMETER_MSN),
				""
			);
